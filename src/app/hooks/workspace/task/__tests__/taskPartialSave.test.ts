...
jest.mock("react", () => ({ ...jest.requireActual("react"), useCallback: jest.fn((callback) => callback), useRef: jest.fn() }));
...
it("retries the saved task and acknowledged dependency after a later blocker write fails", async () => {
  ...
  jest.mocked(useRef).mockReturnValue(guard);
  ...
  jest.mocked(createTask).mockImplementation(() => new Promise((resolve) => { finishCreate = () => resolve(saved); }));
