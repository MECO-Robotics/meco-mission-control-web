...
it("retains authoritative blocking flags and QA risk proposals after bootstrap conversion", () => {
  ...
  const report = { id: "report-1", reportType: "QA" as const, projectId: bootstrap.projects[0].id, taskId: bootstrap.tasks[0].id, milestoneId: null, workstreamId: null, createdByMemberId: null, result: "pass", summary: "Reassess", notes: "Reviewed", photoUrl: "", createdAt: "2026-09-08", targetRiskId: "risk-1", proposedRiskSeverity: "low" as const, proposedRiskStatus: "full-mitigation" as const };
  ...
  expect(normalized.qaReports[0]).toMatchObject({ targetRiskId: "risk-1", proposedRiskSeverity: "low", proposedRiskStatus: "full-mitigation" });
});
