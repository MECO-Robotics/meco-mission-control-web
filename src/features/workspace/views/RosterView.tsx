...
import { IconEdit, IconPlus, IconTasks, IconTrash } from "@/components/shared";
...
export const RosterView: React.FC<RosterViewProps> = ({
  ...
  const renderMemberRow = (member: MemberRecord) => {
    ...
    const disciplineName = member.disciplineId
      ? bootstrap.disciplines.find((discipline) => discipline.id === member.disciplineId)?.name ??
        member.disciplineId
      : null;
    ...
    return (
      <div
        className={member.id === selectedMemberId ? "member-row active editable-action-host" : "member-row editable-action-host"}
        key={member.id}
      >
    ...
    <section className={`panel dense-panel roster-layout ${WORKSPACE_PANEL_CLASS}`}>
      ...
      <div className="roster-columns">
        <div className="panel-subsection">
          ...
          <div className="roster-list">{sortedStudents.map(renderMemberRow)}</div>
        </div>
        ...
        <div className="panel-subsection">
          <div className="roster-section-header">
            ...
            <button className="roster-section-add" onClick={() => openAddPersonPanel("mentor")} type="button">
              <IconPlus />
            </button>
          </div>
          <div className="roster-list">{sortedMentors.map(renderMemberRow)}</div>
        </div>
        ...
        <div className="panel-subsection">
          <div className="roster-section-header">
            ...
            <button className="roster-section-add" onClick={() => openAddPersonPanel("external")} type="button">
              <IconPlus />
            </button>
          </div>
          <div className="roster-list">{sortedExternalMembers.map(renderMemberRow)}</div>
        </div>
      </div>
          ...
          <section aria-modal="true" className="modal-card roster-edit-modal" role="dialog">
            ...
            <form className="compact-form roster-inline-form" onSubmit={handleAddMemberSubmit}>
              ...
              <div className="modal-actions modal-wide">
                <button className="secondary-action" onClick={closeAddPersonPopup} type="button">
                  Cancel
                </button>
                <button
          ...
          <section aria-modal="true" className="modal-card roster-edit-modal" role="dialog">
            ...
            <form className="compact-form roster-inline-form" onSubmit={handleUpdateMember}>
              ...
              <label className="field">
                <span>Name</span>
                <input
                  onChange={(e) => setMemberEditDraft((curr) => (curr ? { ...curr, name: e.target.value } : null))}
                ...
                <FilterDropdown
                  ...
                  onChange={(selection) => {
                    setMemberEditDraft((curr) =>
                      curr ? { ...curr, disciplineId: selection[0] ?? null } : null,
                    );
                  }}
                  options={disciplineOptions}
                  singleSelect
              ...
              <div className="modal-actions modal-wide">
                ...
                <button className="secondary-action" onClick={closeEditPersonPopup} type="button">
                  Cancel
                </button>
